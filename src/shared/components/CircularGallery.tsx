...
class Title {
  ...
  constructor({ gl, plane, text, textColor = "#545050", font = "30px sans-serif" }: TitleOptions) {
    this.gl = gl;
    this.plane = plane;
    this.text = text;
...
class App {
  ...
  constructor(container: HTMLElement, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase }: AppOptions) {
  ...
  addEventListeners() {
    ...
    // Wheel/drag start listeners are scoped to the gallery container (not
    // `window`) so this component doesn't hijack scrolling on the rest of
    // the page when it's mounted off-screen.
