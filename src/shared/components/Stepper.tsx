...
interface RenderStepIndicatorArgs {
  step: number;
  currentStep: number;
...
interface StepperProps {
  ...
  /** When true, the final step's action returns to step 1 instead of collapsing the stepper. */
  loop?: boolean;
          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? "spread" : "end"}`}>
              ...
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps}>
                {isLastStep && !loop ? "Complete" : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: "#141414", color: "#8c8c8c" },
